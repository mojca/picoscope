...
* History:
*
*	10Dec13 - Updated wrapper to copy streaming data in callback.
*   11Dec13 - Added HasOverflowed function.
*   16Dec13 - update data types for buffers
*
***************************************************************************/
#include <windows.h>
#include <stdio.h>
...
*
****************************************************************************/
extern int16_t __declspec(dllexport) __stdcall RunBlock(int16_t handle, int32_t preTriggerSamples, int32_t postTriggerSamples, 
            uint32_t timebase, int16_t oversample, uint16_t segmentIndex)
{
	_ready = 0;
	_numSamples = preTriggerSamples + postTriggerSamples;

	return (int16_t) ps4000RunBlock(handle, preTriggerSamples, postTriggerSamples, timebase, oversample, 
...
*
****************************************************************************/
extern int16_t __declspec(dllexport) __stdcall GetStreamingLatestValues(int16_t handle)
{
	_ready = 0;
	_numSamples = 0;
	_autoStop = 0;

	return (int16_t) ps4000GetStreamingLatestValues(handle, StreamingCallback, &_wrapBufferInfo);
}
...
*
****************************************************************************/
extern PICO_STATUS __declspec(dllexport) __stdcall SetTriggerConditions(int16_t handle, int32_t *conditionsArray, int16_t nConditions)
{
	PICO_STATUS status;
	int16_t i = 0;
	...
	for (i = 0; i < nConditions; i++)
	{
		...
		conditions[i].pulseWidthQualifier			= (TRIGGER_STATE) conditionsArray[5 * (i+1)];
		conditions[i].aux							= (TRIGGER_STATE) conditionsArray[6 * (i+1)];
	}
...
*
****************************************************************************/
extern PICO_STATUS __declspec(dllexport) __stdcall SetRapidBlockDataBuffers(int16_t handle, uint16_t channel, int16_t *buffer, uint16_t nCaptures, int32_t nSamples)
{
	uint16_t capture;
...
*
****************************************************************************/
extern int16_t __declspec(dllexport) __stdcall HasOverflowed(int16_t handle)
{
	return _overflow;
}
...
****************************************************************************/
extern short __declspec(dllexport) __stdcall setEnabledChannels(int16_t handle, int16_t * enabledChannels)
{
...
* -1, otherwise
****************************************************************************/
extern short __declspec(dllexport) __stdcall setAppAndDriverBuffers(int16_t handle, int16_t channel, int16_t * appBuffer, int16_t * driverBuffer, int32_t bufferLength)
{
	if(handle > 0)
	{
		if(channel < 0 || channel >= PS4000_MAX_CHANNELS)
		{
			return -1;
		}
		else
...
* -1, otherwise
****************************************************************************/
extern short __declspec(dllexport) __stdcall setMaxMinAppAndDriverBuffers(int16_t handle, int16_t channel, int16_t * appMaxBuffer, int16_t * appMinBuffer, int16_t * driverMaxBuffer, int16_t * driverMinBuffer, int32_t bufferLength)
{
	if(handle > 0)
	{
		if(channel < 0 || channel >= PS4000_MAX_CHANNELS)
