...
* History:
*
*	11Dec13 - Data types updated, data copied in streaming callback.
*	13Mar14 - 1.0.1.1 Copy data to equivalent size application buffer.
*
***************************************************************************/
#include <windows.h>
#include <stdio.h>
...
// uint32_t g_appBufferStartIndex = 0; // Start index for writing data into application buffer
// uint32_t g_totalSamples = 0;
// int16_t g_appBufferFull = 0;
...
*
****************************************************************************/
extern int16_t __declspec(dllexport) __stdcall RunBlock(int16_t handle, uint32_t preTriggerSamples, uint32_t postTriggerSamples, 
...
* -1, otherwise
****************************************************************************/
extern int16_t __declspec(dllexport) __stdcall setAppAndDriverBuffers(int16_t handle, int16_t channel, int16_t * appBuffer, int16_t * driverBuffer, uint32_t bufferLength)
{
	if(handle > 0)
	{
...
* -1, otherwise
****************************************************************************/
extern int16_t __declspec(dllexport) __stdcall setMaxMinAppAndDriverBuffers(int16_t handle, int16_t channel, int16_t * appMaxBuffer, int16_t * appMinBuffer, int16_t * driverMaxBuffer, int16_t * driverMinBuffer, uint32_t bufferLength)
{
	if(handle > 0)
	{
		if(channel < PS6000_CHANNEL_A || channel >= PS6000_MAX_CHANNELS)
...
*
****************************************************************************/
extern void __declspec(dllexport) __stdcall clearStreamingParameters(int16_t handle)
{
	_ready = 0;
	_autoStop = 0;
	_numSamples = 0;
