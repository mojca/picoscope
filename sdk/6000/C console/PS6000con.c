...
*
* Copyright:   Pico Technology Limited 2010 - 2014
*
* Author:      RPM
*
* Description:
...
* History:
*
*	27Mar14 - Updated data types and signal generator code.
***************************************************************************/
#include <stdio.h>
...
#define BUFFER_SIZE 	100000 // Used for block and streaming mode examples
...
void BlockDataHandler(UNIT * unit, int8_t * text, int32_t offset)
{
	...
	*		 the most suitable time units, and the maximum oversample at the current timebase*/
	while (ps6000GetTimebase(unit->handle, timebase, sampleCount, &timeInterval, oversample, &maxSamples, segmentIndex))
	{
		timebase++;
	}
	...
	if(g_ready) 
	{
		...
		for (i = offset; i < offset+10; i++) 
		{
			for (j = 0; j < unit->channelCount; j++) 
			{
				if (unit->channelSettings[j].enabled) 
				{
					...
					adc_to_mv(buffers[j * 2][i], unit->channelSettings[PS6000_CHANNEL_A + j].range)		// If scaleVoltages, print32_t mV value
					:buffers[j * 2][i]);																															// else print32_t ADC Count
				}
			}
			printf("\n");
		}
		...
		if (fp != NULL)
		{
			...
			{
				fprintf(fp, "%5lld ", g_times[0] + (int64_t)(i * timeInterval));
				for (j = 0; j < unit->channelCount; j++) 
				{
					//fprintf(fp, "%lld ", g_times[j] + (int64_t)(i * timeInterval));
					if (unit->channelSettings[j].enabled) 
					{
						fprintf(fp,
...
void StreamDataHandler(UNIT * unit, uint32_t preTrigger)
{
	...
	for (i = (uint32_t) PS6000_CHANNEL_A; i < unit->channelCount; i++) // create data buffers
	{
		if(unit->channelSettings[i].enabled)
		{
			buffers[i * 2] = (int16_t*) calloc(sampleCount, sizeof(int16_t));
	...
	while (!_kbhit() && !g_autoStopped)
	{
		...
		if (status != PICO_OK)
		{
			//TODO handle buffer/communication errors
		...
		if (g_ready && g_sampleCount > 0) /* can be ready and have no data, if autoStop has fired */
		{
			...
			if (fp != NULL)
			{
				...
				{
					for (j = (uint32_t) PS6000_CHANNEL_A; j < unit->channelCount; j++) 
					{
						if (unit->channelSettings[j].enabled) 
						{
							fprintf(	fp,
	...
	for (i = (uint32_t) PS6000_CHANNEL_A; i < unit->channelCount; i++) 
	{
		if(unit->channelSettings[i].enabled)
		{
			free(buffers[i * 2]);
...
void CollectBlockTriggered(UNIT * unit)
{
	...
	int16_t triggerVoltage = mv_to_adc(1000, unit->channelSettings[PS6000_CHANNEL_A].range);
...
void CollectRapidBlock(UNIT * unit)
{
	...
	while (ps6000GetTimebase(unit->handle, timebase, nSamples, &timeInterval, oversample, &maxSamples, segmentIndex))
	{
		timebase++;
	}
...
void SetSignalGenerator(UNIT * unit)
{
	...
	{
		status = ps6000SetSigGenBuiltIn(unit->handle, offset, pkpk, waveform, (float)frequency, (float)frequency, 0, 0, 
...
{
	int16_t triggerVoltage = mv_to_adc(100,	unit->channelSettings[PS6000_CHANNEL_A].range); // ChannelInfo stores ADC counts
	struct tPwq pulseWidth;
